...
export function useContent(loc) {
  ...
  async function fetchMostRecent() {
    waitFor(async () => {
      ...
      return await checkCache(
        collection,
        cache.size(collection) >= limit.value,
        async () => await getMostRecent(locale.value, limit.value)
      );
    });
  }
  ...
  async function fetchOneContent(id) {
    waitFor(async () => {
      return await checkCache(
        "content",
        cache.hasItem("content", id),
        async () => await findOneContent(id)
      );
    });
  }
  ...
  async function fetchAllContent(ids) {
    waitFor(async () => {
      return await checkCache(
        "content",
        cache.hasItems("content", ids),
        async () => await findAllContent(ids)
      );
    });
  }
  ...
  async function fetchTranslationsForContent(id) {
    waitFor(async () => {
      ...
      return await checkCache(
        collection,
        cache.hasItem(collection, id),
        async () => await findAllTranslationsForContent(id)
      );
    });
  }
