    ...
    <div class="relative max-w-6xl m-auto">
      <div class="text-center">
        <h2 class="text-3xl tracking-tight font-extrabold sm:text-4xl">
          <span class="text-near-green">{{ t('collections.single.title') }} </span><span class="text-gray-500">{{ collection.name }}</span>
        </h2>
        <p v-if="collectionStatus === 'ready'" v-html="renderMarkdown(collection.description)" class="mt-3 max-w-6xl mx-auto text-xl sm:mt-4 text-gray-500"></p>
      </div>
    </div>
    <div class="relative max-w-6xl m-auto">
      <main>
        <div class="mt-8 block">
          <div class="align-middle inline-block min-w-full">
            ...
            <table v-if="collection.total_posts" class="min-w-full">
              <thead>
                <tr>
                  <th class="px-6 pt-8 pb-4 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <span class="lg:pl-2">Videos</span>
                  </th>
                  <th class="hidden sm:table-cell px-6 pt-8 pb-4 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Authors</th>
                  <th class="hidden md:table-cell px-6 pt-8 pb-4 border-b border-gray-200 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Created On</th>
                  <th class="hidden md:table-cell px-6 pt-8 pb-4 border-b border-gray-200 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Last updated</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-100 align-top">
                <tr v-for="video in content" :key="video.id" @click="$router.push({ name: 'video', params: { id: video.id } })" class="hover:bg-near-yellow-light cursor-pointer">
                  <td class="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-600">
                    ...
                    <div class="mt-2 ml-10">
                      ...
                      <span v-for="tag in video.tags" :key="tag" class="inline-flex items-center ml-2 px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-800">
                        {{ tag }}
                      </span>
                    </div>
                    <div class="mt-2 ml-10">
                      <span class="text-gray-400">translations:</span>
                      <span v-for="lang in video.languages" :key="lang" class="inline-flex items-center ml-2 px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-800">
                        {{ lang }}
                      </span>
                    </div>
                  </td>
            ...
            <div v-else>
              <div class="mx-auto text-center py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
                <h2 class="text-3xl font-extrabold tracking-tight text-gray-400 sm:text-4xl">
                  <span class="block">Interested in making this collection?</span>
...
import { PlayIcon, VideoCameraIcon, UserGroupIcon, TranslateIcon } from '@heroicons/vue/outline';
...
export default {
  ...
  setup(props) {
    ...
    const { collections: collection, fetchOneCollection, status: collectionStatus } = useCollections();
    const { content, fetchAllContent, setLimit, status: contentStatus } = useContent();
