<template>
  <div class="relative pt-6 px-4 sm:px-6 lg:pb-28 lg:px-8">
    <div class="relative max-w-6xl m-auto">
      <ul v-if="collections" class="mt-12 max-w-lg mx-auto grid gap-x-4 gap-y-6 sm:grid-cols-2 lg:grid-cols-4 lg:max-w-none">
        <li v-for="collection in collections" :key="collection.id" @click="$router.push({ name: 'collection', params: { id: collection.id } })" class="col-span-1 flex shadow-sm rounded-md cursor-pointer">
          <div :class="[colorizeCollection(collection.total_posts), 'flex-shrink-0 flex items-center justify-center w-16 text-sm font-medium rounded-l-md']">
            {{ t('collections.meal', collection.total_posts) }}
          </div>
          <div class="flex-1 flex justify-between border-t border-r border-b border-gray-light hover:border-gray-300 hover:bg-gray-100 bg-white rounded-r-md truncate w-56">
