...
<template>
  ...
  <div class="w-full flex justify-center mt-8">
    <ul class="max-w-7xl grid grid-cols-1 gap-y-6 gap-x-12 sm:grid-cols-2 lg:grid-cols-3">
      <li v-for="video in videoDetails" :key="video.content" class="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
        <!-- Goes to Loom website for now -->
        <router-link :to="{ name: 'video', params: { id: video.fields.content[0] } }">
          <div class="w-full flex items-center justify-between p-6 space-x-6">
            <div class="flex-1 truncate">
              <div class="flex items-center space-x-3">
                ...
                <span class="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">{{ video.fields.content_tags.toString().replaceAll(',', ' ') }}</span>
              </div>
